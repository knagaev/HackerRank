...
long triplets(vector<int> a, vector<int> b, vector<int> c)
{
    ...
    vector<int> va(sa.begin(), sa.end());
    vector<int> vb(sb.begin(), sb.end());
    vector<int> vc(sc.begin(), sc.end());

    auto ub_va = va.begin();
    auto ub_vc = vc.begin();

    for (const auto& vbi : vb)
    {
        auto new_ub_va = upper_bound(ub_va, va.end(), vbi);
        auto new_ub_vc = upper_bound(ub_vc, vc.end(), vbi);

        if (new_ub_va == va.begin() || new_ub_vc == vc.begin()) continue;

        triplets += distance(va.begin(), new_ub_va) * distance(vc.begin(), new_ub_vc);
        //cout << vai << " " << *itb << " " << *ub_vc << " : " << dst_vc << endl;
