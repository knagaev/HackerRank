...
{
    vector<int> parents(t_nodes + 1);
    vector<int> childrenQty(t_nodes + 1);

    for (size_t i = 0; i < t_from.size(); ++i)
    {
        const auto& child = t_from[i];
        const auto& parent = t_to[i];
    ...
    int result = 0;
    for (size_t i = t_nodes; i > 1; --i)
    {
        if (childrenQty[i] % 2 != 0)
        {
            --childrenQty[parents[i]];
            result++;
        }
    }
